...
export function buildBackupPayload(plan, plans, records, activePlanId) {
  ...
  return {
    ...
    plans: safePlans,
    activePlanId: activePlanId || plan?.id || safePlans[0]?.id || null,
    plan,
...
export function downloadBackupJson(plan, plans, records, activePlanId) {
  const payload = buildBackupPayload(plan, plans, records, activePlanId)
